...
def copy_random_list(head):
    ...
    while curr is not None:
        node_dict[curr].next = node_dict[curr.next] if curr.next is not None else None
        node_dict[curr].random = node_dict[curr.random] if curr.random is not None else None

    return node_dict[head] if head is not None else None